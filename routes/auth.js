...
exports.register = async (req,res) => { // Register
        const {firstname,lastname,username,email,telephone,password,userrole} = req.body;   //Register 
        try {
            const user = await User.findOne({email}); // check wheather there is a user with the requested email
        if(!user)
        {
            const user = await User.create({
                firstname,lastname,username,email,telephone,password,userrole}); //if not, create the user
                sendToken(user,201,res);
        }        
        else
        {
            return res.status(400).json ({success:false, error:"Email in use. Please provide new email"});
        }       
}
         catch (error)
         {
...
exports.login = async  (req,res) => { //Login
    ...
    if(!email || !password){
       return  res.status(422).json ({success:false, error:"Please provide email and password"});  // bad request  
    }
...
exports.resetPassword = async (req,res) => {    //Reset Password
    const resetPasswordToken = crypto.createHash("sha256").update(req.params.resetToken).digest("hex"); // compare the token in url and hashed version
